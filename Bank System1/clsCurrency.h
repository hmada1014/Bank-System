...
public:
    ...
    static  float CalculateCurrency(clsCurrency Currency1, float Amount)
    {
        clsCurrency CurrencyUSD = clsCurrency::FindByCode("USD");
        float Result = 0;
     
        if (Currency1.CurrencyCode() != CurrencyUSD.CurrencyCode())
        {
                return Result = (Amount / Currency1.Rate());
        }
        else
        {
            return Result = (Amount * CurrencyUSD.Rate());
        }
